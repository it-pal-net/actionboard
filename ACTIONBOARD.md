...
| `packages/excalidraw/components/App.tsx` | hooks (3 imports + 4 call sites: `handleSelectionOnPointerDown` top + before the `resize.handleType` branch, the hover block in `handleCanvasPointerMove`, and the embeddable-hint guard in `renderEmbeddables`) | connector dots start arrows, corner ring arms rotation, hover cursors — `packages/excalidraw/actionboard/connectorDots.ts` + `packages/excalidraw/actionboard/cornerRotation.ts`; card embeds render no "Click to interact" hover hint — `packages/excalidraw/actionboard/cardEmbeds.ts` |
...
| `packages/excalidraw/context/tunnels.ts` | hook (one tunnel entry in type + init) | `ABToolbarExtraTunnel` — host slot beside the shapes toolbar; component in `packages/excalidraw/actionboard/ToolbarExtra.tsx` |
| `packages/excalidraw/components/LayerUI.tsx` | hook (one `<tunnels.ABToolbarExtraTunnel.Out />` in the toolbar `Stack.Row`, after the laser island) | renders `<ToolbarExtra>` children beside the toolbar (desktop layout only — MobileMenu untouched) |
| `packages/excalidraw/index.tsx` | hook (import + export line) | exports `ToolbarExtra` to consumers |
...
| `packages/excalidraw/components/HintViewer.tsx` | branch removed in `getHints` | the persistent idle hint (`hints.canvasPanning`, "To move canvas, hold …") no longer renders; contextual hints (linear editing, resize, crop, …) stay |
